...
describe("InternalHandler contract", function () {
  ...
  async function resetContracts() {
    ...
    handler = await InternalHandler.new(HNItoken_controller.address);
    ...
    // Use account3,account4 as mock HNItokens
  ...
  describe("Deployment", function () {
    it("Should deployed and only initialized once", async function () {
      ...
      await truffleAssert.reverts(
        handler.initialize(HNItoken_controller.address, {
          from: owner,
        }),
  ...
  describe("setHNITokenController", function () {
    it("Should only allow auth to set HNITokenController", async function () {
      let new_HNItoken_addresses = await HNITokenController.new();
      await handler.setHNITokenController(new_HNItoken_addresses.address);

      await truffleAssert.reverts(
        handler.setHNITokenController(new_HNItoken_addresses.address, {
          from: account1,
        }),
    ...
    it("Should not allow set HNITokenController with the same address", async function () {
      await truffleAssert.reverts(
        handler.setHNITokenController(await handler.HNITokenController()),
