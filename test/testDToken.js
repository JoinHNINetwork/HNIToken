...
describe("HNIToken Contract", function () {
  ...
  async function resetContracts(handler_num, proportions) {
    ...
    for (i = 0; i < handler_num; i++) {
      let h = await InternalHandler.new(HNItoken_controller.address);
    ...
    await HNItoken_controller.setHNITokensRelation([USDC.address], [dUSDC.address]);
  ...
  describe("Mint", function () {
    ...
    it("Should not mint HNItoken with the smallest unit underlying token when exchange rate > 1", async function () {
      ...
      // Now try to mint the smallest unit underlying token whose value is < 1 HNItoken
      await truffleAssert.reverts(
    ...
    it("Check mint the smallest unit of underlying token when exchange rate < 1", async function () {
      ...
      // Now try to mint 1 underlying token whose value is > 1 HNItoken
      await dUSDC.mint(account1, 1, {from: account1});
      ...
      // User would exchange 1 underlying token to 1 HNIToken as it's floored
    ...
    it("Check mint with some interest and changing exchange rate", async function () {
      ...
      for (let i = 0; i < iteration; i++) {
        ...
        // Get the underlying token balance in HNIToken
  ...
  describe("Redeem", function () {
    ...
    it("Should redeem the smallest unit of HNItoken when exchange rate >= 1", async function () {
      await dUSDC.mint(account1, 10e6, {from: account1});
    ...
    it("Check redeem the smallest unit of HNItoken when exchange rate < 1", async function () {
      let proportions = [200000, 200000, 200000, 200000, 200000];
      await resetContracts(5, proportions);
      await dUSDC.mint(account1, 500e6, {from: account1});
      ...
      // Now try to redeem 1 HNItoken whose value is < 1 underlying token
      await dUSDC.redeem(account1, 1, {from: account1});
      ...
      // User would get 0 underlying token while 1 HNIToken was burned
  ...
  describe("Redeem Underlying", function () {
    ...
    it("Check redeem the smallest unit of underlying token when exchange rate < 1", async function () {
      ...
      // Now try to redeem 1 underlying token whose value is > 1 HNItoken
      await dUSDC.redeemUnderlying(account1, 1, {from: account1});
      ...
      // User would get 1 underlying token while 2 HNIToken was burned
      assert.equal(usdc_diff.toString(), 1);
  ...
  describe("transfer/transferFrom", function () {
    ...
    it("Should be able to transfer HNIToken", async function () {
    ...
    it("Should be able to transferFrom HNIToken", async function () {
    ...
    it("Should not be able to transferFrom HNIToken more than its balance", async function () {
      await truffleAssert.reverts(
    ...
    it("Should not be able to transferFrom HNIToken from unapproved account", async function () {
      await truffleAssert.reverts(
  ...
  describe("Approve/increaseAllowance/decreaseAllowance", function () {
    ...
    it("Should be able to transfer others HNIToken when approved", async function () {
  ...
  describe("balanceOf", function () {
    ...
    it("Should get 0 as the initial HNIToken balance of account", async function () {
      assert.equal((await dUSDC.balanceOf(account1)).toString(), 0);
    });

    it("Should be able to get HNIToken balance of account", async function () {
      await dUSDC.mint(account1, 1000e6, {from: account1});
  ...
  describe("getExchangeRate", function () {
    ...
    it("Should get 0 when there is no token in handlers", async function () {
      // Mint some HNItoken and mock some interest to make the exchange rate go up to 2
      await dUSDC.mint(account1, 1000e6, {from: account1});
      await USDC.allocateTo(handler_addresses[0], 1000e6);
