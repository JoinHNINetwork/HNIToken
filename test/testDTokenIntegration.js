...
describe("HNIToken Contract Integration", function () {
  ...
  async function resetContracts() {
    ...
    await HNItoken_controller.setNHITokensRelation(
  ...
  describe("HNIToken Integration: Only internal handler", function () {
    before(async function () {
    ...
    it("Case 14: Should be able to pause HNIToken", async function () {
      await dUSDC.pause();
    });

    it("Case 15: Should not be able to mint/redeem/redeemUnderlying after HNIToken is paused", async function () {
      await truffleAssert.reverts(
        dUSDC.mint(account1, 1000e6, {from: account1}),
    ...
    it("Case 16: Should not be able to transfer after HNIToken is paused", async function () {
      await truffleAssert.reverts(
    ...
    it("Case 17: Should not be able to unpause HNIToken", async function () {
      await dUSDC.unpause();
    });

    it("Case 18: Should be able to mint/redeem/redeemUnderlying HNIToken", async function () {
      await dUSDC.mint(account1, 1000e6, {from: account1});
      await dUSDC.redeem(account1, 500e6, {from: account1});
      await dUSDC.redeemUnderlying(account1, 500e6, {from: account1});
    });

    it("Case 19: Should be able to transfer HNIToken", async function () {
      let balance2 = await dUSDC.balanceOf(account2);
  ...
  describe("HNIToken Integration: internal and compound handler ", async function () {
    before(async function () {
      await resetContracts();
    });
    ...
    // 25. mint some dusdc
    it("Case 25: Should mint/redeem/redeemUnderlying some HNIToken", async function () {
      let amount = new BN(1000e6);
      let diff;
    ...
    it("Case 28: should mint some HNIToken with fee", async function () {
      let amount = new BN(1000e6);
      let diff;
    ...
    it("Case 32: Should pause Compound handler", async function () {
      //Mint some HNIToken for later use
    ...
    it("Case 38: Should be able to pause HNIToken", async function () {
      await dUSDC.pause();
      await dUSDT.pause();
    });

    it("Case 39: Should fail on Mint/Redeem/RedeemUnderlying when HNIToken paused", async function () {
      await truffleAssert.reverts(
        dUSDC.mint(account1, 2000e6, {from: account1}),
        "whenNotPaused: paused"
      );
    ...
    it("Case 40: Should not be able to transfer when HNIToken paused", async function () {
      await truffleAssert.reverts(
    ...
    it("Case 41: Should unpause HNIToken", async function () {
      await dUSDC.unpause();
      await dUSDT.unpause();
    });

    it("Case 42: Should be able to mint/redeem/redeemUnderlying HNIToken", async function () {
      await dUSDC.mint(account1, 2000e6, {from: account1});
      await dUSDT.mint(account1, 2000e6, {from: account1});
      await dUSDC.redeem(account1, 1000e6, {from: account1});
      await dUSDT.redeem(account1, 1000e6, {from: account1});
  ...
  describe("HNIToken Integration: internal, compound and avee handler ", async function () {
    before(async function () {
      await resetContracts();
    });
    ...
    it("Case 57: Normal executions with two handlers", async function () {
      ...
      let remained_HNIToken_balance = (
        await dUSDC.balanceOf(account1)
      ).toString();
      await dUSDC.redeem(account1, remained_HNIToken_balance, {from: account1});
    ...
    it("Case 69: Transfer 5 dusdc", async function () {
      ...
      let original_HNItoken_balance = (
        await dUSDC.balanceOf(account1)
      ).toString();
      await dUSDC.transfer(account2, transfer_amount.toString(), {
        from: account1,
      });
      let current_HNItoken_balance = (await dUSDC.balanceOf(account1)).toString();
      assert.equal(
        original_HNItoken_balance - current_HNItoken_balance,
    ...
    it("Case 72: Should pause HNIToken", async function () {
      await dUSDC.pause();
      await dUSDT.pause();
    ...
    it("Case 73: When pause the HNITokenController, mint-redeem-redeemUnderlying will fail", async function () {
      let mint_amount = new BN(7 ** 9);
    ...
    it("Case 74: When pause the HNITokenController, transfer HNIToken will fail", async function () {
      await truffleAssert.reverts(
    ...
    it("Case 75: Unpause HNIToken", async function () {
      await dUSDC.unpause();
      await dUSDT.unpause();
  ...
  describe("HNIToken Integration: Fee related cases ", async function () {
    before(async function () {
      await resetContracts();
      await dUSDC.updateOriginationFee(REDEEM_SELECTOR, FEE);
      await dUSDC.updateOriginationFee(MINT_SELECTOR, FEE);
  ...
  describe("HNIToken Integration: Rebalance related cases ", async function () {
    before(async function () {
      await resetContracts();
      await dispatcher.resetHandlers(
        [internal_handler.address, compound_handler.address],
