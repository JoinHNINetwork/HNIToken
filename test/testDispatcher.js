...
const mock_HNItoken = "0x0000000000000000000000000000000000000001";
...
describe("Dispatcher Contract", function () {
  ...
  async function resetContracts(handler_num, proportions) {
    ...
    await HNItoken_controller.setHNITokensRelation([USDC.address], [mock_HNItoken]);
  ...
  describe("addHandlers", function () {
    ...
    it("Should only allow auth to add handler", async function () {
      let new_handlers = [
        (await InternalHandler.new(HNItoken_controller.address)).address,
      ];
