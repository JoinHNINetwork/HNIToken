...
contract CompoundHandler is Handler, ReentrancyGuard {
    ...
    constructor(address _HNITokenController, address _compAddress) public {
        initialize(_HNITokenController, _compAddress);
    }

    // --- Init ---
    // This function is used with contract proxy, do not modify this function.
    function initialize(address _HNITokenController, address _compAddress)
        public
    {
    ...
    /**
     * @dev Authorized function to approves market and HNIToken to transfer handler's underlying token.
    ...
    /**
     * @dev Internal function to transfer COMP airdrops to corresponding HNIToken to distribute.
     */
    function claimComp(address _underlyingToken) internal {
        ...
        if (compBalance > 0) {
            ...
            require(
                doTransferOut(compAddress, _HNIToken, compBalance),
